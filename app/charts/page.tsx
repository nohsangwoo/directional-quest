...
export default function ChartsPage() {
  ...
  type ConsumptionRow = { cups: number; teams: Record<string, TeamMetrics> }
  ...
  function toConsumptionCombined(data: unknown): {
    points: ConsumptionRow[]
    teams: string[]
  } {
  ...
  const { points: consumptionData, teams: consumptionTeams } =
    toConsumptionCombined(consumption.data as unknown)
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={consumptionData}>
                ...
                <Tooltip />
                <Legend />
                {(consumptionTeams.length
                  ? consumptionTeams
                  : (['Frontend', 'Backend', 'AI', 'Mobile'] as string[])
                ).map((team, idx) => (
                  <Fragment key={team}>
                    <Line
                      yAxisId="left"
                      type="monotone"
                      dataKey={`teams.${team}.bugs`}
                      name={`${team} bugs`}
                    ...
                    <Line
                      yAxisId="right"
                      type="monotone"
                      dataKey={`teams.${team}.productivity`}
